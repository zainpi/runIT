...
The store exposes these API routes:

- `GET /api/templates/config/` reports whether Stripe, the included AI editor, and the optional app icon generator are configured, whether the key is in test mode, and the currency for the validated request origin. It returns no credentials or prompt content.
- `POST /api/templates/checkout/` validates same-origin JSON, one to six distinct catalog IDs, optional boolean `subagents`, `skillTree`, and `appIcon` selections that default to false for legacy clients, and a browser-generated 256-bit access token. The current storefront always supplies all three booleans explicitly. It creates an idempotent Stripe Checkout Session and returns the hosted Checkout URL.
- `POST /api/templates/library/` requires the Checkout Session ID and the original access token. It retrieves the order from Stripe, verifies store/version metadata, the recorded CAD or USD currency and exact amount including independently selected add-ons, completed Checkout status, and the access-token hash before returning purchased prompt foundations. Paid orders also require a paid status and valid charge without a refund or dispute; 100% gift orders require verified gift metadata, a zero total, a completed status and no Payment Intent. It returns subagent or skill-tree instructions only when the verified order metadata includes the corresponding purchase, and an `appIcon` entitlement for the on-site icon generator.
...
- `POST /api/templates/referral/` validates an optional discount code and returns its percentage and attribution. The private gift code applies 100% to the selected order.
- `POST /api/templates/ai/` loads, generates, applies (paid only), or deletes a plan after verifying a paid order or redeemed trial.
- `POST /api/templates/icon/` loads saved icon versions, reserves an initial generation or update, downloads a selected PNG, or deletes saved icon content. Every action requires a verified paid order with the app icon add-on.
...
Stripe is the durable order ledger. A separate SQLite-backed Durable Object now stores AI conversations, icon versions, and usage per purchase, while Checkout Session metadata records the purchased template IDs, catalog version, access-token hash, store marker, pricing currency/origin, and delivery state. Payment Intent metadata identifies the store, template IDs, add-ons, and pricing currency/origin. The browser retains the private access token in the fragment of the success URL; URL fragments are not sent to the server in HTTP requests. Store pages and API responses set `Referrer-Policy: no-referrer` and `X-Content-Type-Options: nosniff`. All API responses use private no-store caching. The public store/library shells contain no paid content. The store renders dynamically so its initial prices and metadata match the request hostname; the browser then uses the same-origin configuration response.

On `runsit.ca` (including `www`), prices are CAD $9.99 for the first template, CAD $5.00 for each additional distinct template, CAD $5.00 for the optional subagent workflow, CAD $10.00 for the optional skill tree, and CAD $5.00 for app icon creation. On `runs-it.com` (including `www`), the same numeric prices are charged in USD. Local development and configured staging origins default to CAD. This is fixed domain pricing, not exchange-rate conversion. Each add-on is charged once for the whole order. Subagents and skill setup apply to every purchased template; icon creation includes one initial icon plus three updates shared across the order. No add-on is multiplied by template count. Prices are computed server-side as 999 cents plus 500 cents for every additional template, plus 500 cents once for subagents, 1,000 cents once for the skill tree, and 500 cents once for app icon creation when selected. Checkout uses inline `price_data` in the server-selected currency and explicitly disables Adaptive Pricing to keep the charged currency aligned with the storefront. It also explicitly sets `managed_payments.enabled=false`: this store uses standard Checkout, whose custom submit text, fixed currency, and exact-total verification are incompatible with an account defaulting to Managed Payments. runsIT retains responsibility for applicable taxes; Stripe is not the merchant of record for these sessions. Adopting Managed Payments requires updating the product tax codes, pricing presentation, and paid-order verification as a separate integration change. The `templates-standard-v2-` idempotency prefix keeps the revised request separate from older saved-cart requests while keeping subsequent retries stable. Library access independently recomputes and verifies both subtotal and total against the recorded `currency` and `pricing_origin` metadata. Legacy sessions lacking both pricing metadata fields remain valid only in USD. A buyer reopening a paid link is verified against the original order terms, regardless of the current storefront domain. Checkout idempotency and browser pending-cart keys include the currency, so an old USD checkout cannot be reused for a new CAD order. The browser supplies only catalog IDs, the private token, three boolean add-on choices, and an optional referral code; it cannot supply prices, currency, arbitrary Stripe Price IDs, success URLs, or paid prompt content. Verified Stripe metadata, rather than localStorage or receipt fields, authorizes delivery of each paid add-on.

Founder referral codes are available for the three co-founders: `ZAIN-RUNIT-10` (Zain Piyarali), `RAI-RUNIT-10` (Raid Shakih), and `MIKAEL-RUNIT-10` (Mikael Siddiqui). Each applies 10% off the full order, including selected add-ons. Codes are normalized case-insensitively, validated server-side, and recorded as founder attribution plus a discount digest in Stripe metadata. The browser may preview the discount, but it cannot set prices or create a discounted Checkout Session without a valid code.

A private high-entropy gift code applies 100% off the full selected bundle, including add-ons. Only its SHA-256 digest is in source; the plaintext code must stay private. It is reusable while active. To stop new redemptions, disable code matching but retain the historical digest for existing orders. The server creates zero-priced Stripe line items, and access is granted only after Checkout completes with a verified zero total, matching gift metadata, `no_payment_required` or `paid` status, and no Payment Intent. Ordinary paid orders still require a valid charge. Mocked tests cover the free completion and fulfillment path; confirm it once in Stripe test mode before relying on it live.

The storefront has six compact template choices and one order panel. Buyers can proceed directly to payment after selecting a template; the app brief, working mode, and model guide live in the purchased library. Optional extras and discount codes use collapsed disclosures, while a separate short form handles free-trial codes. The mobile order bar shows the current total and jumps to the order panel, then hides when the panel is visible. The draft remembers all three optional choices alongside the cart and any existing brief, including across a canceled checkout. The private library enables “Include subagent workflow” and “Include skill tree setup” by default only for the verified entitlements on the active order. Buyers may omit an entitled workflow without losing the purchase, and switching between manual and computer-control modes preserves the choices. The skill-tree purchase also exposes separate Copy skill setup prompt and Download skill setup `.txt` actions so its installation guidance can be used independently from the application build prompt. Switching to another saved order recalculates all entitlements and clears any content that order did not purchase, including the icon preview and history.
...
The customer fills in the brief and explicitly selects the OpenAI consent checkbox before generating. The initial response contains an overview, concise feature descriptions, proposed technical details, assumptions and unresolved questions. Messages revise that proposed specification. The customer UI shows technical details in place of assumptions; older saved plans receive template-based technical starting points without spending another message. **Apply plan to my prompt** saves the reviewed specification and adds it to the locally composed download without replacing the fixed foundation, build mode or purchased add-ons. Merely generating a revision does not apply it. Changing the manual brief excludes a previously applied plan until the customer updates/reviews it. The saved AI brief and applied plan restore across devices. Manual form changes remain browser-local until submitted to AI.
...
Generation uses OpenAI Responses with strict structured output, `store:false`, no tools, the selected template's **public catalog metadata**, the submitted brief, the current plan and the latest six conversation entries. The provider receives no paid foundation, subagent workflow, skill-tree setup or saved app icons, access token or purchase URL. The model returns an explicit plan/off-topic/restricted/unsafe disposition; the server supplies fixed errors for rejected requests and does not save them or deduct a customer message. Both input and output moderation results must be present and unflagged before a plan is delivered. Responses are validated and rendered as React text, never HTML. Model/provider errors are not logged or returned verbatim. Prompts and messages must not be added to analytics or request-body logs. `store:false` disables stored response state; it is not a promise of zero provider retention. Review [OpenAI data controls](https://developers.openai.com/api/docs/guides/your-data) for the configured project.

### Chat safety plan

1. **Keep authority on the server.** Stripe verification and the private access token determine the purchased template IDs and add-ons. Chat text, browser state and model output never grant entitlements, change quotas, select a model or call tools. Paid copy reaches the browser only through the verified library response.
2. **Keep paid copy out of model context.** The editor needs the public template summary and the customer's product brief to propose a plan. It does not need the purchased foundation or add-on instructions. This removes the direct route for a chat injection to repeat proprietary text.
3. **Constrain the conversation.** The server-owned instructions define app planning as the only task. Briefs, saved plans, history and current messages are untrusted JSON data. The structured disposition rejects requests for hidden prompts, secrets, paid content, unrelated tasks and unsafe material. Rejected attempts leave the reviewed plan and customer message allowance unchanged; the existing attempt ceiling still limits repeated provider calls.
4. **Check output before delivery.** Responses request `omni-moderation-latest` results for input and output. A missing/error result fails closed. Flagged content, malformed output and invalid plan structure cannot be saved, applied or returned. Applying a valid plan still requires a separate customer action.
5. **Verify in staging.** Run synthetic entitlement and adversarial tests locally, then use a controlled live-provider staging set covering role spoofing, encoded instructions, paid add-on requests, off-topic chat, unsafe input and safe app edits. Review false positives and outputs before enabling sales. Prompting and moderation reduce risk but cannot prove that every future injection will fail; keep the feature narrow and revisit the test set after model or prompt changes.

The input/output moderation behavior follows the [official OpenAI moderation guide](https://developers.openai.com/api/docs/guides/moderation); the prompt-injection and adversarial-testing approach follows [OpenAI safety best practices](https://developers.openai.com/api/docs/guides/safety-best-practices).
...
- `TEMPLATES_AI_MODEL=gpt-6-luna`: the owner-selected model, supporting Responses structured output. The runtime still requires an explicit model value.
...
Every icon action re-verifies the Stripe payment, add-on metadata, exact total, private token, refund/dispute state and same-origin request. Trial tokens and forged local receipts grant no access. The API accepts no client image bytes, model, provider URL, price or quota. Only a saved image from the verified order can be used as the base version. The customer consents before sending a brief or selected image to OpenAI. The provider receives public template context, the app name/description/style, the icon direction, and (for updates) the selected PNG; it receives no payment identifiers, private URL, credentials, paid foundations or chat history.

`TemplateAiOrder` stores icon metadata and image chunks separately from the existing chat state. A transaction saves the reservation and a Durable Object alarm together before returning HTTP 202. The alarm performs the provider request even after the browser leaves. A 180-second provider timeout and a 240-second watchdog lease fence late completions. Duplicate alarms never repeat an in-flight call. A failed attempt does not consume a completed generation or update; explicit retries are capped at three attempts per version, with at most twelve provider attempts for an order. After that attempt limit, existing versions remain available and support must investigate. Successful completion appends an immutable version and atomically advances the allowance.

PNG data is limited to 6 MiB per image and stored as base64 in 256 KiB SQLite chunks keyed by version; only the selected image is returned with the small version list. This respects the [Durable Object SQLite limits](https://developers.cloudflare.com/durable-objects/platform/limits/). Jobs use [Durable Object alarms](https://developers.cloudflare.com/durable-objects/api/alarms/). Deleting saved icons removes every image and saved brief without resetting usage or request fingerprints, and prevents further updates because no base image remains. Deleting chat does not affect icons, and deleting icons does not affect chat. Preserve this usage ledger and deletions when restoring storage.

Configuration uses the existing `TEMPLATES_AI_ORDERS` binding and `TEMPLATES_OPENAI_API_KEY` secret, with `TEMPLATES_ICON_ENABLED=true` and `TEMPLATES_ICON_MODEL=gpt-image-2.5-flare-2026-09-08`. The icon switch is independent of the text AI switch. No new namespace or class migration is needed; the existing object's constructor creates its icon tables. The image provider follows the [OpenAI image generation and editing API](https://developers.openai.com/api/docs/guides/image-generation), requesting one opaque 1024 × 1024 high-quality PNG with automatic moderation. New icon purchases and generations require valid configuration; previously saved images can still be read, downloaded or deleted while generation is paused. Readiness checks configuration only, so staging still needs a real image generation and edit with the intended project key before sales are enabled.

## Free-trial code

Choosing **Have a free-trial code?** switches the order panel to a short trial form with a code, template, description, consent, and one completion button. The direct `/templates/#free-trial` link opens the same form. Back to checkout preserves the paid cart and extras. Customers must choose a template, write a description, and consent to OpenAI processing before redemption. The existing brief is reused; a single selected template is preselected, while an empty or multiple-template cart requires an explicit choice. The server validates the brief and consent before consuming a code use. Each redemption includes **one free overview plus 3 editing messages**.

After checkout, `/templates/trial/#session_id=…&access=…` opens a project dashboard. The saved brief starts its free overview automatically; the resulting overview, two-sided feature cards and expandable technical details appear beside the AI chat. The chat shows up to three unresolved decisions at a time. Yes, No, or a short written answer adds each choice to the current draft without removing other changes; the answered card slides out and the next decision appears. Submitted answers stay resolved in the saved conversation. Replies update the plan immediately. Phones use a Plan / Chat switch; feature cards flip on tap or keyboard activation as well as hover. Private-link recovery, plan/chat export and content deletion remain available. Existing links without a saved brief have a compact setup instead of the storefront form. Full foundations, add-ons and applying a plan to a purchased build prompt remain locked. The trial model receives only the public catalog description; paid foundation content never enters its context. Trial and purchased conversations remain separate; customers can export their trial decisions.

Checkout seeds the brief and consent into the existing per-trial AI object, keyed identically to the AI endpoint. Initialization is idempotent: a retry cannot overwrite the brief or resurrect deleted content. The registry contains no copy of the brief. Automatic overview generation uses a stable request ID across tabs and starts only before any generation attempt; failed generation requires a manual retry. Pending responses are polled, and lost HTTP responses recover from saved history without sending another message. These optional JSON state fields need no new Durable Object migration.
...
- **Mobile app:** combines reusable foundations from `/Users/Zain/Developer/PulseDeals`, its API in this repository, and `/Users/Zain/Developer/recipie`. The 2026-09-23 update draws from PulseDeals' `DesignSystem.swift`, `Components.swift`, `OnboardingView.swift`, `MainTabView.swift`, `Models.swift`, `AppState.swift`, and `Services.swift`: native navigation and semantic surfaces, welcome/tailoring flows, the compact tier-and-period plan picker, verified membership recovery, account state cleanup, notification preferences, rule synchronization, and notification tap routing. The foundation also retains local-first editing, Share Extension persistence, revision conflicts, durable imports, provider adapters, and consented AI proposals. Product-specific deal and structured-content workflows remain optional generic modules.
...
The downloadable foundation now specifies complete SwiftUI screens and states for onboarding, navigation, subscription selection, Profile/Settings, and notifications. Subscription tiers and billing periods are separate configuration, with localized StoreKit prices, real introductory-offer eligibility, restore/manage flows, and recovery from pending purchases or interrupted verification. Account handling includes session recovery, isolation, sign-out cleanup, and a complete deletion/revocation flow. Notification settings must persist and affect delivery; permission, APNs registration, server connection, local sample alerts, and real delivery are explicitly separate states.

Native Liquid Glass is an addition guided by Apple's documentation, beyond PulseDeals' existing semantic card/material styling. The prompt uses standard navigation controls and iOS 26 availability checks for custom `glassEffect`/`GlassEffectContainer` controls, with an older-system fallback and accessibility requirements. Plan comparisons and content use readable conventional surfaces. API availability was checked against Apple's documentation on 2026-09-23: [Liquid Glass adoption](https://developer.apple.com/documentation/technologyoverviews/adopting-liquid-glass), [custom glass views](https://developer.apple.com/documentation/swiftui/applying-liquid-glass-to-custom-views), [SubscriptionStoreView](https://developer.apple.com/documentation/storekit/subscriptionstoreview), and [account deletion](https://developer.apple.com/support/offering-account-deletion-in-your-app/).

The public catalog summarizes these capabilities. The full implementation instructions remain server-only. Accounts, subscriptions, notifications, and other integrations still follow the buyer's brief. Plan names, prices, product IDs and branding must be configured for the buyer's app; no source-app configuration or user data ships with the foundation. The extra recovery and deletion requirements are implementation guidance for new apps, not a claim that every source-app integration is complete or live.

Local verification: all 13 template content, sanitization and composition checks passed, alongside TypeScript, ESLint and `git diff --check`. This update changes the downloadable prompt and its catalog description; no iOS app build, simulator launch, live purchase, APNs delivery or deployment was performed.

## Prompt builder preview

The homepage introduces an upcoming prompt builder for people making multiple apps. The template store keeps a single coming-soon enquiry link at `/templates/#prompt-builder` below checkout. It displays no price or billing schedule, and the enquiry email contains no pricing. The link does not start checkout or subscribe anyone. No prompt quota, unlimited usage, AI generation credits or cancellation policy is promised.
...
Before production, add request-rate limiting for `/api/templates/checkout/`, `/api/templates/library/`, `/api/templates/ai/`, `/api/templates/icon/` and `/api/templates/trial/` in Cloudflare WAF. The application currently validates origin, input shape and size, but it does not provide a durable distributed rate limiter. Apply conservative per-IP limits with a documented support bypass or tuning procedure, test them against ordinary checkout retries, and monitor blocks. The webhook route should remain reachable by Stripe and rely on signature verification; do not apply a browser-only same-origin rule to it.
...
1. Verify both public domains: one template costs $9.99 CAD on `.ca` and $9.99 USD on `.com`; six cost $34.99 in that domain's currency. Skill-tree-only totals are $19.99 and $44.99; subagent-only totals are $14.99 and $39.99; selecting both produces $24.99 and $49.99. Adding app icon creation adds $5.00 once; all three add-ons produce $29.99 and $54.99. Each add-on appears once per order regardless of template count.
...
9. Checkout always sends all three boolean add-on flags but no client amount or prompt text. Refresh and cancellation preserve the independent selections. Verified orders expose only their purchased workflows and separate skill-setup copy/download artifact; forged browser storage cannot expose an unpurchased add-on, and selecting another saved order clears any entitlement it lacks.
10. An icon purchase includes one original plus three updates. Update an earlier version, reopen the private link, download that version, and confirm a fourth update is denied. Lost responses recover through status refresh without consuming another update. Failed provider attempts preserve existing versions and the remaining update allowance.
...
## Trial dashboard verification on 2026-09-17

The trial flow now requires a template, description and AI consent before completing a $0 checkout. Its dashboard restores the server-saved brief, starts the included overview, and places the editable feature plan beside chat. Mobile views keep Plan / Chat navigation accessible and bring the composer into view. The paid library and purchase allowance are unchanged.

- Backend suite: 59 passing checks, including validation before redemption, recovery after initialization failure, durable brief/consent restoration, and deletion that cannot be reversed by a checkout retry. The existing 50-use cap and quota checks still pass.
- Browser coverage: 22 existing storefront/paid-library cases and 8 trial dashboard cases passed across the full regression and focused runs. Trial cases cover required checkout fields, lost-response recovery, automatic overview behavior, three editing messages, downloads after exhaustion, a clean-browser return, legacy links, pending-result polling, and mobile errors/layout.
- TypeScript, lint, the Next.js/OpenNext production build, and a local Wrangler dry-run passed. Screenshots use synthetic BoulderMe content; no real provider request or trial redemption was made.
- This change is local only. No Worker deployment or production configuration change was performed.

## App icon verification on 2026-09-23

- `npm run test:templates`: 75 checks passed, including exact icon pricing and entitlements, three-update enforcement, reference-image edits, failed-attempt recovery, and persisted version history across an actual SQLite Durable Object restart. Provider requests were intercepted with synthetic PNG responses.
- All 35 browser cases passed across the full run and corrected currency-label reruns. The three icon cases also passed after adding a lost-response check. They cover on-site generation, earlier-version selection, all three updates, selected-version downloads, clean-browser restoration, deletion, order switching and mobile layout.
- TypeScript, lint, the Next.js/OpenNext production build and Wrangler dry-run passed. The existing compatibility-date and third-party bundle warnings remain. Desktop/mobile screenshots were inspected using a synthetic image fixture.
- Trial Download plan buttons now appear at the top and bottom with matching lime styling; the bottom button's download is covered by the trial browser test.
- No deployment, real payment, or live image-provider request was made. Validate initial image generation and an edit with the configured OpenAI project in staging before enabling sales.

## Simplified checkout verification on 2026-09-23

- Checkout now needs only a template selection before payment. Extras, discount codes, template details, and the two help answers expand on demand. The trial form collects its code, template, idea, and consent together.
- All 35 browser cases passed across the full run and focused rerun. Checks cover CAD/USD totals and discounts, all three extras, saved drafts and canceled carts, trial validation and retry recovery, direct trial links and returning to checkout, keyboard controls, and desktop/mobile layout.
- TypeScript, lint, `git diff --check`, and the Next.js production build passed. Desktop, mobile, and trial screenshots were visually reviewed with synthetic data.
- Changes remain local; no deployment or live payment was made.
