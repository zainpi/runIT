...
The store has seven API routes:
...
- `POST /api/templates/referral/` validates an optional founder referral code and returns only its 10% discount and founder attribution.
...
Entering a **Free-trial code** in the order summary switches it to a $0 checkout with one clear completion button. Customers must choose a template, write a description, and consent to OpenAI processing before redemption. The existing brief is reused; a single selected template is preselected, while an empty or multiple-template cart requires an explicit choice. The server validates the brief and consent before consuming a code use. Each redemption includes **one free overview plus 3 editing messages**.

After checkout, `/templates/trial/#session_id=…&access=…` opens a project dashboard. The saved brief starts its free overview automatically; the resulting overview, feature table, assumptions and questions appear beside the AI chat. Replies update the plan immediately. Phones use a Plan / Chat switch. Private-link recovery, plan/chat export and content deletion remain available. Existing links without a saved brief have a compact setup instead of the storefront form. Full foundations, add-ons and applying a plan to a purchased build prompt remain locked. The trial model receives only the public catalog description; paid foundation content never enters its context. Trial and purchased conversations remain separate; customers can export their trial decisions.
