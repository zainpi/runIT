-- Standalone PostgreSQL. Apply as the dedicated Neutronium database owner.
create table public.neutronium_users (
 id uuid primary key, email text not null unique check(email=lower(email)),
 password_hash text, verified boolean not null default false,
 created_at timestamptz not null default now()
);
create table public.neutronium_sessions (
 token_hash text primary key, user_id uuid not null references public.neutronium_users(id),
 expires_at timestamptz not null
);
create index on public.neutronium_sessions(user_id);
create table public.neutronium_auth_tokens (
 token_hash text primary key, user_id uuid not null references public.neutronium_users(id),
 kind text not null check(kind in ('signup','invite','recovery')), expires_at timestamptz not null
);
...
create table public.neutronium_memberships (
 ...
 user_id uuid not null references public.neutronium_users(id), role text not null check (role in ('ORG_OWNER','ORG_ADMIN','HR_ADMIN','MANAGER','APPROVER','EMPLOYEE')),
...
create table public.neutronium_platform_members (
 user_id uuid primary key references public.neutronium_users(id), role text not null check (role in ('PLATFORM_OWNER','PLATFORM_SUPPORT','PLATFORM_SECURITY','PLATFORM_READONLY'))
);
...
create table public.neutronium_oauth_states (
 id text primary key, organization_id uuid not null references public.neutronium_organizations(id), user_id uuid not null references public.neutronium_users(id),
...
-- No public SQL execution; the application connects as the dedicated database owner.
