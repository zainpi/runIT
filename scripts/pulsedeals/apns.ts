...
async function providerToken(): Promise<string> {
  ...
  const keyID = getPulseDealsEnv("PULSEDEALS_APNS_KEY_ID");
  const teamID = getPulseDealsEnv("PULSEDEALS_APNS_TEAM_ID");
  const pem = getPulseDealsEnv("PULSEDEALS_APNS_PRIVATE_KEY")?.replace(/\\n/g, "\n");
...
export async function sendPush(request: PushRequest): Promise<PushResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const stream = session.request({
      ...
      "apns-topic": getPulseDealsEnv("PULSEDEALS_BUNDLE_ID") ?? "com.pulsedeals.app",
