...
export async function dispatchOnce(admin: SupabaseClient, sender: (request: PushRequest) => Promise<PushResult> = sendPush): Promise<number> {
  const claimed = await admin.rpc("claim_pulsedeals_push_deliveries", { p_limit: 5 });
  ...
  for (const delivery of (claimed.data ?? []) as Row[]) {
    ...
    const update = async (patch: Row) => {
      const result = await admin.from("pulsedeals_push_deliveries").update(patch)
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        if (invalidDevice) {
          const disabled = await admin.from("pulsedeals_push_devices").update({ enabled: false }).eq("id", device!.id).eq("token", device!.token);
  ...
  const expired = await admin.from("pulsedeals_push_deliveries").update({ state: "cancelled", last_error: "expired" })
    .in("state", ["pending", "sending"]).lt("expires_at", new Date().toISOString());
  if (expired.error) throw expired.error;
  const exhausted = await admin.from("pulsedeals_push_deliveries").update({ state: "failed", last_error: "retries_exhausted" })
...
async function main() {
  const supabaseURL = getPulseDealsEnv("PULSEDEALS_SUPABASE_URL")?.trim() || getPulseDealsEnv("NEXT_PUBLIC_SUPABASE_URL")?.trim();
  if (!supabaseURL) throw new Error("Missing configuration: PULSEDEALS_SUPABASE_URL");
  for (const name of ["SUPABASE_SERVICE_ROLE_KEY", "PULSEDEALS_APNS_KEY_ID", "PULSEDEALS_APNS_TEAM_ID", "PULSEDEALS_APNS_PRIVATE_KEY"]) {
    if (!getPulseDealsEnv(name)) throw new Error(`Missing configuration: ${name}`);
  }
  const admin = createClient(supabaseURL, getPulseDealsEnv("SUPABASE_SERVICE_ROLE_KEY")!, {
