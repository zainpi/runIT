...
import { entitlementStatus, verifyNotification, verifyTransaction } from "@/lib/pulsedeals/apple";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const duplicate = await admin.from("pulsedeals_apple_notifications").select("notification_uuid").eq("notification_uuid", notification.notificationUUID).maybeSingle();
    if (duplicate.error) throw duplicate.error;
    if (duplicate.data) return apiJson(request, { ok: true, duplicate: true });

    const signedTransactionInfo = notification.data?.signedTransactionInfo;
    if (!signedTransactionInfo) {
      await admin.from("pulsedeals_apple_notifications").insert({
    ...
    if (transaction.appAccountToken) {
      const account = await admin.from("pulsedeals_accounts").select("id").eq("app_account_token", transaction.appAccountToken).maybeSingle();
      if (account.error) throw account.error;
      accountID = account.data?.id ?? null;
    }
    if (!accountID) {
      const entitlement = await admin.from("pulsedeals_entitlements").select("account_id").eq("original_transaction_id", transaction.originalTransactionId).maybeSingle();
    ...
    const upsert = await admin.rpc("record_pulsedeals_apple_entitlement", {
    ...
    const notificationInsert = await admin.from("pulsedeals_apple_notifications").insert({
      notification_uuid: notification.notificationUUID,
      notification_type: notification.notificationType ?? null,
    });
