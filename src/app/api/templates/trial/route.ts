import { AiError, parseBrief } from "@/lib/templates/ai-contract";
...
export async function POST(request: Request) {
  try {
    ...
    if (!data.brief || typeof data.brief !== "object" || !("idea" in data.brief) || typeof data.brief.idea !== "string" || !data.brief.idea.trim()) throw new AiError("Describe what you want to make before checking out.");
    const brief = parseBrief(data.brief);
    if (data.consent !== true) throw new AiError("Allow OpenAI to use your brief to prepare your overview.");
    ...
    const sessionId = `trial_${accessHash}`;
    const initialized = await ai.orders.getByName(tokenHash(sessionId)).initializeTrial(brief);
    if (!initialized.ok) throw new AiError(initialized.error, initialized.status);
    return jsonResponse({ sessionId, templateId: result.value.templateId, messageLimit: TRIAL_MESSAGE_LIMIT });
  } catch (error) {
