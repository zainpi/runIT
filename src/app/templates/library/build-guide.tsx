...
export function BuildGuide({ project, details, title, foundation, mode, subagents, skillTree }: { project: AiProject; details: Personalization; title: string; foundation: string; mode: BuildMode; subagents?: string; skillTree?: string }) {
  ...
  const name = artifact.brief.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 70) || "my-app";
  return <section className={guideStyles.guide} aria-labelledby="build-guide-heading">
    ...
    <p className={styles.small}>Saved from plan revision {artifact.sourceRevision} · {artifact.generatedAt.slice(0, 10)} · Export uses your current {mode === "computer" ? "computer control" : "manual"} mode and enabled purchased add-ons.</p>
    ...
    <div className={styles.actions}><button className={styles.primary} onClick={() => downloadText(html, `${name}-complete-guide.html`, "text/html;charset=utf-8")}>Download complete HTML guide</button><button className={styles.secondary} aria-expanded={preview} aria-controls="guide-preview" onClick={() => setPreview(!preview)}>{preview ? "Close guide preview" : "Preview guide & prototype"}</button></div>
