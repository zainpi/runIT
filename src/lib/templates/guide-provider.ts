...
Preserve the reviewed plan's requirements and requested platform. questionChoices contains unselected suggestions, not requirements; questions remain unresolved until the customer answers them. Resolve missing reversible choices with labeled defaults. Put decisions that block implementation in needs-answer, with a recommendation and why. Never silently remove a feature. Distinguish first release from later ideas. Default connected mobile/web apps to Supabase for Postgres, Auth, Storage and server functions unless the brief already chooses another backend. Explain platform-specific exceptions (e.g. Roblox authoritative server/DataStore, Discord bot hosting, offline games); do not add a backend to an offline product without a reason. Supabase does not host the client or replace Apple/Google accounts, payment providers, email delivery or push services.
