...
const replySchema = {
  ...
  properties: { disposition: { type: "string", enum: ["plan", "off_topic", "restricted", "unsafe"] }, message: string, plan: {
    ...
    properties: { overview: string, features: { type: "array", items: { type: "object", additionalProperties: false, required: ["part", "description"], properties: { part: string, description: string } } }, assumptions: strings, questions: strings, questionChoices: {
      type: "array", maxItems: 8, items: { type: "object", additionalProperties: false, required: ["question", "options"], properties: {
        question: string, options: { type: "array", minItems: 2, maxItems: 2, items: string },
      } },
    }, technicalDetails: strings },
  } },
};
