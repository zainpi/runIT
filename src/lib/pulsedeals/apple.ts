import { getPulseDealsEnv } from "./compatibility";
...
async function verifierFor(environment: Environment): Promise<SignedDataVerifier> {
  ...
  const appAppleID = getPulseDealsEnv("PULSEDEALS_APPLE_ID")
    ? Number(getPulseDealsEnv("PULSEDEALS_APPLE_ID"))
    : undefined;
  if (environment === Environment.PRODUCTION && !appAppleID) {
    throw new Error("Missing server configuration: PULSEDEALS_APPLE_ID");
  }
...
export async function verifyRenewalInfo(jws: string, environment: Environment) {
  return (await verifierFor(environment)).verifyAndDecodeRenewalInfo(jws);
}
