import { getPulseDealsEnv, isPulseDealsSessionType } from "./compatibility";
...
import {
  ...
  type PulseDeal,
  type PulseDealVote,
  type PulseDealVoteSummary,
  type PulseMarketplace,
  type PulseDiscordConnection,
...
function getRequiredEnv(name: string): string {
  const value = getPulseDealsEnv(name);
...
export async function isSupportedAppleProduct(admin: SupabaseClient, productID?: string): Promise<boolean> {
  ...
  const result = await admin.from("pulsedeals_product_tiers").select("product_id").eq("product_id", productID).maybeSingle();
...
export function getDiscordConfig(): DiscordConfig {
  const required = [
    "PULSEDEALS_DISCORD_CLIENT_ID",
    "PULSEDEALS_DISCORD_CLIENT_SECRET",
    "PULSEDEALS_DISCORD_BOT_TOKEN",
    "PULSEDEALS_DISCORD_GUILD_ID",
    "PULSEDEALS_DISCORD_ROLE_ID",
  ];
  const missing = required.find((name) => !getPulseDealsEnv(name));
  if (missing) throw new Error(`Missing server configuration: ${missing}`);

  const paidRoles = [getPulseDealsEnv("PULSEDEALS_DISCORD_PAID_ROLE_ID"), getPulseDealsEnv("PULSEDEALS_DISCORD_PRO_ROLE_ID")].filter(Boolean);
  if (paidRoles.includes(getPulseDealsEnv("PULSEDEALS_DISCORD_ROLE_ID"))) throw new Error("Paid Discord roles must differ from the app access role");
  return {
    clientID: getPulseDealsEnv("PULSEDEALS_DISCORD_CLIENT_ID")!,
    clientSecret: getPulseDealsEnv("PULSEDEALS_DISCORD_CLIENT_SECRET")!,
    botToken: getPulseDealsEnv("PULSEDEALS_DISCORD_BOT_TOKEN")!,
    guildID: getPulseDealsEnv("PULSEDEALS_DISCORD_GUILD_ID")!,
    roleID: getPulseDealsEnv("PULSEDEALS_DISCORD_ROLE_ID")!,
    redirectURI: getPulseDealsEnv("PULSEDEALS_DISCORD_REDIRECT_URI")
      ?? "https://runsit.ca/pulsedeals/api/v1/discord/callback",
    serverName: getPulseDealsEnv("PULSEDEALS_DISCORD_SERVER_NAME") ?? "PulseDeals",
  };
}
...
export async function beginDiscordLink(
  ...
  const insert = await admin.from("pulsedeals_discord_link_states").insert({
  ...
  await admin.from("pulsedeals_discord_link_states").delete().lt("expires_at", new Date().toISOString());
...
async function consumeDiscordLinkState(admin: SupabaseClient, state: string): Promise<string> {
  ...
  const result = await admin
    .from("pulsedeals_discord_link_states")
...
async function ensureDiscordLinkAvailable(
  ...
  if (discordLink.data && discordLink.data.account_id !== accountID) {
    throw new Error("This Discord account is already linked to another PulseDeals account");
  }
}
...
async function saveDiscordLink(
  ...
  activeSubscription: boolean,
): Promise<PulseDiscordConnection> {
  const roles = member.roles ?? [];
...
export type Membership = { tier: "none" | "standard" | "pro"; source: "none" | "apple" | "discord"; primaryMarketplace: PulseMarketplace | null; expiresAt: string | null };

export function paidDiscordTier(member: DiscordGuildMember | null): "standard" | "pro" | null {
  if (!member || member.pending) return null;
  const roles = member.roles ?? [];
  const accessRole = getPulseDealsEnv("PULSEDEALS_DISCORD_ROLE_ID");
  const proRole = getPulseDealsEnv("PULSEDEALS_DISCORD_PRO_ROLE_ID");
  const paidRole = getPulseDealsEnv("PULSEDEALS_DISCORD_PAID_ROLE_ID");
...
async function readMembership(admin: SupabaseClient, accountID: string): Promise<Membership> {
  const result = await admin.rpc("pulsedeals_membership", { p_account_id: accountID });
  if (result.error) throw result.error;
...
export async function getMembership(admin: SupabaseClient, accountID: string, forceDiscord = false): Promise<Membership> {
  const link = await admin.from("pulsedeals_discord_links").select("membership_checked_at").eq("account_id", accountID).maybeSingle();
...
export async function refreshDiscordMemberships(admin: SupabaseClient): Promise<void> {
  const result = await admin.from("pulsedeals_discord_links").select("account_id")
...
export async function getDiscordConnection(
  admin: SupabaseClient,
  accountID: string,
): Promise<PulseDiscordConnection | null> {
  const result = await admin
    .from("pulsedeals_discord_links")
    .select("id, account_id, discord_user_id, username, global_name, avatar_hash, guild_id, membership_status, is_pending, has_access_role, access_granted, linked_at, updated_at")
    .eq("account_id", accountID)
  ...
  const grant = await admin.from("pulsedeals_discord_links").update({
  ...
  const updated = await admin.from("pulsedeals_discord_links").update({
    guild_id: config.guildID,
    membership_status: member ? (member.pending ? "pending" : "member") : "not_member",
    is_pending: Boolean(member?.pending),
...
export async function unlinkDiscord(admin: SupabaseClient, accountID: string): Promise<void> {
  ...
  const deleted = await admin.from("pulsedeals_discord_links").delete().eq("account_id", accountID);
...
async function signSession(claims: SessionClaims): Promise<string> {
  const secret = getRequiredEnv("PULSEDEALS_SESSION_SECRET");
...
async function verifySession(token: string): Promise<SessionClaims> {
  ...
  if (decoded.header.alg !== "HS256" || !isPulseDealsSessionType(decoded.payload.typ)) {
...
export async function verifyAppleIdentityToken(token: string): Promise<AppleIdentity> {
  ...
  const bundleID = getPulseDealsEnv("PULSEDEALS_BUNDLE_ID") ?? "com.pulsedeals.app";
  if (
    !valid ||
...
export async function createOrGetAccount(
  ...
  if (subLookup.data) {
    ...
    // Keep the account purchase token stable across devices, including Discord-first members.
...
export async function issueSession(admin: SupabaseClient, account: AccountRow): Promise<SessionResponse> {
  ...
  const refreshInsert = await admin.from("pulsedeals_refresh_tokens").insert({
  ...
  return {
    accessToken: await signSession({ sub: account.id, iat: now, exp: now + SESSION_TTL_SECONDS, typ: "pulsedeals-session" }),
    refreshToken,
    expiresAt,
...
export async function refreshSession(admin: SupabaseClient, refreshToken: string): Promise<SessionResponse> {
  ...
  const lookup = await admin
    .from("pulsedeals_refresh_tokens")
...
export async function consumeRateLimit(
  ...
  const result = await admin.rpc("consume_pulsedeals_rate_limit", {
...
export async function claimIdempotency(
  ...
  const result = await admin.rpc("claim_pulsedeals_idempotency", {
...
export function normalizeMarketplace(value: string | null): PulseMarketplace {
  if (value && (PULSE_MARKETPLACES as readonly string[]).includes(value)) return value as PulseMarketplace;
  return "uk";
}

export function normalizeCategory(value: string | null): PulseCategory | null {
  if (value && (PULSE_CATEGORIES as readonly string[]).includes(value)) return value as PulseCategory;
  return null;
}
...
export function mapDeal(row: Record<string, unknown>): PulseDeal {
  return {
    asin: String(row.asin),
    title: String(row.title ?? "Amazon deal"),
    marketplace: normalizeMarketplace(String(row.marketplace ?? "uk")),
...
function emptyDealVoteSummary(): PulseDealVoteSummary {
  return {
    goodVotes: 0,
    boughtVotes: 0,
    badVotes: 0,
...
export async function getDealVoteSummaries(
  ...
  accountID: string,
  asins: string[],
): Promise<Record<string, PulseDealVoteSummary>> {
  ...
  for (const row of (result.data ?? []) as Array<Record<string, unknown>>) {
    ...
    const vote = row.vote as PulseDealVote;
    if (!summary || !(PULSE_DEAL_VOTES as readonly string[]).includes(vote)) continue;
...
export async function submitDealVote(
  ...
  const result = await admin.from("pulsedeals_deal_votes").upsert({
    account_id: accountID,
    asin: normalizedASIN,
    vote,
...
export function handleApiError(request: Request, error: unknown): Response {
  ...
  if (message === "Active subscription required") {
    return apiError(request, 403, "subscription_required", "An active PulseDeals subscription is required.");
  }
  ...
  if (message.includes("Missing server configuration: PULSEDEALS_DISCORD")) {
  ...
  if (message.includes("Missing server configuration")) {
    return apiError(request, 503, "not_configured", "The PulseDeals API is not configured yet.");
  }
  console.error("PulseDeals API error:", error);
  return apiError(request, 500, "server_error", "PulseDeals could not complete that request.");
}
